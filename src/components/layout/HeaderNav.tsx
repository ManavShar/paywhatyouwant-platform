...
import { AccountMenu, SignOutButton, type HeaderUser } from "./AccountMenu";
import { CardMetaSync } from "./CardMetaSync";
import { CATEGORIES } from "@/lib/taxonomy";
import { cn } from "@/lib/utils";

/**
 * The header's markup and its open/closed state.
 *
 * Client-side for the menu and search toggles only — who you are arrives as a
 * prop from `SiteHeader`, already resolved on the server.
 *
    ...
    <header className="sticky top-0 z-50 border-b border-hairline bg-canvas/90 backdrop-blur-md">
        ...
        <nav className="ml-auto hidden items-center gap-1 md:flex">
          ...
          {user ? (
            <>
              ...
              <AccountMenu user={user} className="ml-1" />
            </>
          ) : (
            <>
              <Link
      ...
      {menuOpen && (
        <div className="border-t border-hairline bg-canvas px-4 py-3 md:hidden">
          ...
          {user ? (
            <>
              {/* Signed in, on a phone: say so before offering anything, so the
                  answer to "am I logged in?" does not depend on reading the
                  links and inferring it. */}
              <p className="px-2 pb-1 pt-2 text-xs font-bold uppercase tracking-wide text-ink-subtle">
              ...
              <SignOutButton className="block w-full rounded-control px-2 py-3 text-left text-[0.9375rem] font-medium hover:bg-surface-hover" />
              {isVendor ? (
                <Link
                  href="/dashboard/products/new"
                  onClick={() => setMenuOpen(false)}
